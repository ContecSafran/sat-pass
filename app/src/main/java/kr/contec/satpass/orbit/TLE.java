 ...
 */
package kr.contec.satpass.orbit;
...
public class TLE implements Serializable {
    ...
    public TLE(final String[] tle) throws IllegalArgumentException {
        {
            ...
            catnum = parseAlpha5CatNum(strip(tle[1].substring(2, 7)));
            name = tle[0].trim();
            setnum = Integer.parseInt(strip(tle[1]
                    .substring(64, 68)));
            year = Integer
                    .parseInt(strip(tle[1].substring(18, 20)));
            ...
            orbitnum = Integer.parseInt(strip(tle[2].substring(63,
                    68)));
    ...
    public int parseAlpha5CatNum(String rawSubStr) {
        if (rawSubStr == null || rawSubStr.length() != 5) {
            throw new IllegalArgumentException("Invalid satellite number format. Must be 5 characters.");
        }

        char firstChar = rawSubStr.charAt(0);
        String remaining = rawSubStr.substring(1);

        // 1. 첫 글자가 숫자인 경우: 기존 방식 처리 (00000 ~ 99999)
        if (Character.isDigit(firstChar)) {
            return Integer.parseInt(rawSubStr.trim());
        }

        // 2. 첫 글자가 알파벳인 경우: Alpha-5 디코딩
        if (Character.isUpperCase(firstChar)) {
            int offset = firstChar - 'A';

            // 'I'와 'O'는 제외되므로 오프셋 조정
            if (firstChar > 'O') {
                offset -= 2; // I와 O 둘 다 지나왔으므로 2 차감
            } else if (firstChar > 'I') {
                offset -= 1; // I만 지나왔으므로 1 차감
            }

            // 기본 100,000 + (알파벳 오프셋 * 10,000) + 나머지 4자리 숫자
            int base = 100000 + (offset * 10000);
            int remainingNum = Integer.parseInt(remaining);

            return base + remainingNum;
        }

        throw new NumberFormatException("Invalid character in satellite number: " + firstChar);
    }
    /**
    ...
    /**
     * commons-lang3 의 StringUtils.strip 대체. (predict4java 이식 시 외부 의존 제거)
     *
     * @param str 대상 문자열
     * @return 앞뒤 공백을 제거한 문자열, str 이 null 이면 null
     */
